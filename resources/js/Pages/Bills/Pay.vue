<template>
  <div>
    <Head :title="`Pay Bill - ${bill.bill_number || `Bill #${bill.id}`}`" />
    ...
    <div class="mb-8">
      <h1 class="text-3xl font-bold mb-2">
        ...
        <Link :href="`/bills/${bill.id}`" class="text-brand-400 hover:text-brand-600">{{ bill.bill_number || `Bill #${bill.id}` }}</Link>
        <span class="text-brand-400 font-medium">/</span>
        Pay
      </h1>
      <p class="text-gray-600">Record payment for this bill</p>
    </div>

    <!-- Bill Summary -->
    <div class="bg-white rounded-lg shadow p-6 mb-8">
      <h2 class="text-lg font-semibold mb-4">Bill Summary</h2>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div>
          ...
          <div class="text-gray-900 font-semibold">{{ $formatAmount(bill.amount) }}</div>
        </div>
        <div>
          <div class="text-sm font-medium text-gray-500">Amount Paid</div>
          <div class="text-gray-900 font-semibold">{{ $formatAmount(bill.amount_paid || 0) }}</div>
        </div>
        <div>
          <div class="text-sm font-medium text-gray-500">Balance Due</div>
          <div class="text-red-600 font-semibold">{{ $formatAmount(balanceDue) }}</div>
        </div>
        <div>
      ...
      <form @submit.prevent="recordPayment">
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
          <div class="flex items-center space-x-4">
            <Link :href="`/bills/${bill.id}`" class="text-gray-600 hover:text-gray-800">
          ...
          <button
            type="submit"
            :disabled="form.processing || balanceDue <= 0"
...
export default {
  ...
  methods: {
    recordPayment() {
      this.form.post(`/bills/${this.bill.id}/record-payment`, {
        onSuccess: () => {
          this.$inertia.visit(`/bills/${this.bill.id}`)
        }
      })
    }
  }
