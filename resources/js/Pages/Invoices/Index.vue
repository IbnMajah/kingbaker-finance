            ...
            <tr v-for="invoice in invoices.data" :key="invoice.id" class="hover:bg-gray-50">
              ...
              <td class="px-3 py-3 whitespace-nowrap text-center">
                <div class="flex items-center justify-center space-x-1">
                  ...
                  <!-- Print Button -->
                  <Link v-if="canPrintReceipts" :href="`/invoices/${invoice.id}/print`" class="hidden sm:inline-block text-purple-600 hover:text-purple-900" title="Print Invoice">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <!-- Delete Button -->
                  <button v-if="canEditInvoices" @click="destroy(invoice)" class="hidden sm:inline-block text-red-600 hover:text-red-900" title="Delete Invoice">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default {
  ...
  layout: Layout,
  setup() {
    const { canCreateInvoices, canViewInvoices, canEditInvoices, canPrintReceipts, hasPermission } = usePermissions()
    return { canCreateInvoices, canViewInvoices, canEditInvoices, canPrintReceipts, hasPermission }
  },
  props: {
    filters: {
      type: Object,
