        ...
        <div class="flex items-center">
          ...
          <div class="lg:ml-4">
            ...
            <p class="text-lg md:text-xl lg:text-2xl font-semibold text-gray-900">
              <MaskableAmount :value="$formatAmount(summary?.total_amount || 0)" /></p>
          </div>
        </div>
      </div>
            ...
            <tr v-for="sale in sales?.data || []" :key="sale.id" class="hover:bg-gray-50">
              ...
              <td class="px-3 py-3 whitespace-nowrap text-center">
                <div class="flex items-center justify-center space-x-1">
                  <Link :href="`/sales/${sale.id}`" class="text-blue-600 hover:text-blue-900" title="View">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                    </svg>
                  </Link>
                  <Link v-if="canEditSales" :href="`/sales/${sale.id}/edit`" class="text-brand-600 hover:text-brand-900" title="Edit">
...
export default {
  ...
  layout: Layout,
  setup() {
    const { canCreateSales, canEditSales, hasPermission } = usePermissions()
    return { canCreateSales, canEditSales, hasPermission }
  },
  props: {
    filters: {
      type: Object,
